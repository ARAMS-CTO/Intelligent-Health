...
const DeveloperPortal: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-indigo-900">
            ...
            {/* Status Check / Dashboard Mode */}
            <div className="max-w-4xl mx-auto mt-20 border-t border-slate-200 dark:border-slate-700 pt-10">
                <h2 className="text-3xl font-bold text-center mb-8">Already Applied?</h2>

                {!applicationId ? (
                    <div className="max-w-md mx-auto glass-card p-6">
                        <h3 className="font-bold mb-4">Check Application Status</h3>
                        <div className="flex gap-2">
                            <input
                                className="flex-1 bg-white/50 border border-slate-300 rounded-lg px-3 py-2"
                                placeholder="Enter Application ID"
                                id="status-check-id"
                            />
                            <button
                                onClick={() => {
                                    const val = (document.getElementById('status-check-id') as HTMLInputElement).value;
                                    if (val) setApplicationId(val);
                                }}
                    ...
                    <IntegratorDashboard appId={applicationId} />
                )}
            </div>
        </div>
    );
...
// --- Mock Integrator Dashboard Component ---
const IntegratorDashboard = ({ appId }: { appId: string }) => {
    ...
    // Simulate Fetch
    React.useEffect(() => {
        // In real app, fetch /api/partners/application/{appId}
        // Mocking 'Approved' for demo if ID is long enough, else pending
        if (appId.length > 5) setStatus('approved');
    }, [appId]);
    ...
    if (status === 'pending') {
        return (
            <div className="max-w-2xl mx-auto glass-card p-8 text-center">
                ...
                <p className="text-gray-500">We are still reviewing your submission ({appId}).<br />Please check back later.</p>
            </div>
        );
    }
    ...
    if (status === 'rejected') {
        return (
            <div className="max-w-2xl mx-auto glass-card p-8 text-center">
                <div className="text-4xl mb-4">❌</div>
                <h3 className="text-xl font-bold text-red-600">Application Rejected</h3>
                <p className="text-gray-500">Please contact support for more details.</p>
            </div>
        );
    }
    ...
    return (
        <div className="glass-card p-8 animate-fade-in">
            <div className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
                <div>
                    <h3 className="text-2xl font-bold text-indigo-700">Integrator Console</h3>
                    <p className="text-sm text-gray-500">ID: {appId}</p>
                </div>
                <span className="px-3 py-1 bg-green-100 text-green-700 font-bold rounded-full text-sm">Active Partner</span>
            </div>

            <div className="grid md:grid-cols-3 gap-6">
                {/* KYC Module */}
                <div className={`p-6 rounded-xl border ${kycStatus ? 'bg-green-50 border-green-200' : 'bg-orange-50 border-orange-200'}`}>
                    <div className="text-3xl mb-4">{kycStatus ? '🛡️' : '⚠️'}</div>
                    <h4 className="font-bold mb-2">KYC Verification</h4>
                    <p className="text-xs text-gray-600 mb-4">{kycStatus ? 'Identity Verified & Compliant' : 'Verification Required for API Access'}</p>
                    <button
                        onClick={() => setKycStatus(true)}
                        disabled={kycStatus}
                        className={`w-full py-2 rounded-lg font-bold text-sm ${kycStatus ? 'bg-green-200 text-green-800' : 'bg-orange-600 text-white hover:bg-orange-700'}`}
                    >
                        {kycStatus ? 'Verified' : 'Start Verification'}
                    </button>
                </div>

                {/* License Module */}
                <div className={`p-6 rounded-xl border ${licenseActive ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                    <div className="text-3xl mb-4">📜</div>
                    <h4 className="font-bold mb-2">Commercial License</h4>
                    <p className="text-xs text-gray-600 mb-4">{licenseActive ? 'Enterprise Plan Active' : 'No active license found.'}</p>
                    <button
                        onClick={() => setLicenseActive(true)}
                        disabled={licenseActive}
                        className={`w-full py-2 rounded-lg font-bold text-sm ${licenseActive ? 'bg-blue-200 text-blue-800' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                    >
                        {licenseActive ? 'Active' : 'Purchase License'}
                    </button>
                </div>

                {/* API Keys Module */}
                <div className="p-6 rounded-xl border bg-slate-50 border-slate-200 opacity-100">
                    <div className="text-3xl mb-4">🔑</div>
                    <h4 className="font-bold mb-2">SDK Keys</h4>
                    <p className="text-xs text-gray-600 mb-4">Manage your API credentials.</p>

                    {kycStatus && licenseActive ? (
                        <div className="bg-black/80 p-3 rounded-lg overflow-hidden relative group cursor-pointer">
                            <code className="text-green-400 text-xs break-all">sk_live_51M...92a</code>
                            <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap text-white text-xs font-bold">
                                Copy Key
                            </div>
                        </div>
                    ) : (
                        <div className="text-xs text-red-500 font-bold bg-red-50 p-2 rounded">
                            Complete KYC & License to view keys.
                        </div>
                    )}
                </div>
            </div>
