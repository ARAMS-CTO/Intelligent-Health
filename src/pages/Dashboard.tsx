        ...
        <div className="glass-card rounded-[32px] p-8 antigravity-target transition-all duration-300 shadow-xl border border-white/20 dark:border-slate-700/50 relative overflow-hidden group">
            ...
            <div className="relative">
                ...
                <div className="absolute bottom-3 right-3">
                    <VoiceInput onTranscript={(text) => setSymptoms(prev => (prev ? prev + " " : "") + text)} />
                </div>
            </div>
...
// Patient Queue Widget - Shows upcoming patients
const PatientQueueWidget: React.FC = () => {
    const [patients] = useState([
        { id: '1', name: 'Emily Carter', age: 45, reason: 'Follow-up consultation', time: '09:00 AM', status: 'waiting', urgent: false },
        { id: '2', name: 'James Wilson', age: 62, reason: 'Chest pain evaluation', time: '09:30 AM', status: 'in-progress', urgent: true },
        { id: '3', name: 'Sofia Rodriguez', age: 28, reason: 'Prenatal checkup', time: '10:00 AM', status: 'scheduled', urgent: false },
        { id: '4', name: 'Michael Brown', age: 55, reason: 'Diabetes management', time: '10:30 AM', status: 'scheduled', urgent: false },
    ]);

    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'waiting': return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300';
            case 'in-progress': return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300';
            default: return 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300';
        }
    };

    return (
        <div className="glass-card rounded-[32px] p-6 border border-white/20 dark:border-slate-700/50 relative overflow-hidden">
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-primary/5 blur-3xl rounded-full"></div>
            <div className="flex items-center justify-between mb-4">
                <h4 className="font-heading font-black text-base text-text-main flex items-center gap-2">
                    <span className="p-1.5 bg-primary/10 rounded-lg text-primary text-sm">👥</span>
                    Patient Queue
                </h4>
            ...
            <div className="space-y-2">
                {patients.slice(0, 4).map((patient) => (
                    <div
                        key={patient.id}
                        className={`flex items-center gap-3 p-3 rounded-xl transition-all hover:shadow-md cursor-pointer group ${patient.urgent
                            ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800'
                            : 'bg-white/40 dark:bg-slate-800/40 border border-white/20'
                            }`}
                    >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-white text-sm ${patient.urgent ? 'bg-red-500' : 'bg-gradient-to-br from-primary to-indigo-600'
                            }`}>
                            {patient.name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1">
                                <p className="font-bold text-xs text-text-main truncate group-hover:text-primary transition-colors">
                                    {patient.name}
                                </p>
                                {patient.urgent && (
                                    <span className="text-[8px] px-1.5 py-0.5 bg-red-500 text-white rounded-full font-bold">
                                        URGENT
                                    </span>
                                )}
                            </div>
                            <p className="text-[10px] text-slate-500 truncate">{patient.reason}</p>
                        </div>
                        <div className="text-right shrink-0">
                            <p className="text-[10px] font-bold text-primary">{patient.time}</p>
                            <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase ${getStatusBadge(patient.status)}`}>
                                {patient.status === 'in-progress' ? 'IN PROG' : patient.status.toUpperCase()}
                            </span>
                        </div>
                    </div>
                ))}
...
// Clinical Alerts Widget - Shows urgent notifications
const ClinicalAlertsWidget: React.FC = () => {
    const [alerts] = useState([
        { id: '1', type: 'critical', title: 'Critical Lab Result', message: 'Troponin levels elevated', time: '2m' },
        { id: '2', type: 'warning', title: 'Medication Interaction', message: 'Review Rx - potential interaction', time: '15m' },
        { id: '3', type: 'info', title: 'Consultation Request', message: 'Dr. Chen requests cardiology consult', time: '1h' },
    ]);

    const getAlertStyle = (type: string) => {
        switch (type) {
            case 'critical': return { bg: 'bg-red-50 dark:bg-red-900/20', border: 'border-red-200 dark:border-red-800', icon: '🚨' };
            case 'warning': return { bg: 'bg-amber-50 dark:bg-amber-900/20', border: 'border-amber-200 dark:border-amber-800', icon: '⚠️' };
            default: return { bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-200 dark:border-blue-800', icon: 'ℹ️' };
        }
    };
    ...
    return (
        <div className="glass-card rounded-[32px] p-6 border border-white/20 dark:border-slate-700/50 relative overflow-hidden">
            <div className="flex items-center justify-between mb-4">
                <h4 className="font-heading font-black text-base text-text-main flex items-center gap-2">
                    <span className="p-1.5 bg-red-500/10 rounded-lg text-red-500 text-sm">🔔</span>
                    Clinical Alerts
                </h4>
                <span className="text-[10px] font-bold text-red-500 bg-red-100 dark:bg-red-900/30 px-2 py-0.5 rounded-lg animate-pulse">
            ...
            <div className="space-y-2">
                {alerts.map((alert) => {
                    ...
                    return (
                        <div
                            key={alert.id}
                            className={`p-3 rounded-xl ${style.bg} border ${style.border} hover:shadow-md transition-all cursor-pointer group`}
                        >
                            <div className="flex items-start gap-2">
                                <span className="text-sm shrink-0">{style.icon}</span>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        <h5 className="font-bold text-xs text-text-main group-hover:text-primary transition-colors truncate">{alert.title}</h5>
                                        <span className="text-[10px] text-slate-400 shrink-0">{alert.time}</span>
                                    </div>
                                    <p className="text-[10px] text-slate-600 dark:text-slate-400 truncate">{alert.message}</p>
                                </div>
                            </div>
                        </div>
                    );
...
// Quick Stats Bar for Doctor Dashboard
const QuickStatsBar: React.FC = () => {
    const stats = [
        { label: 'Patients Today', value: '12', icon: '👥', trend: '+2', color: 'from-blue-500 to-cyan-500' },
        { label: 'Pending Labs', value: '5', icon: '🧪', trend: '-1', color: 'from-purple-500 to-pink-500' },
        { label: 'Messages', value: '8', icon: '💬', trend: '+3', color: 'from-green-500 to-emerald-500' },
        { label: 'Critical Cases', value: '2', icon: '🚨', trend: '0', color: 'from-red-500 to-orange-500' },
    ];

    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {stats.map((stat, idx) => (
                <div key={idx} className={`p-4 rounded-2xl bg-gradient-to-br ${stat.color} text-white relative overflow-hidden group hover:shadow-xl transition-all hover:-translate-y-1`}>
                    <div className="absolute -right-4 -top-4 text-5xl opacity-20">{stat.icon}</div>
...
const Dashboard: React.FC = () => {
        ...
        return enhancedCases
            .filter(caseData => {
                ...
                const matchesSearch = caseData.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    caseData.patientId.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    caseData.assignedSpecialist?.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors duration-500 overflow-x-hidden">
            <Modal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} title="Clinic & Service Settings">
                <DoctorServiceSettings />
            </Modal>

            <div className="flex-none p-6 md:p-10 space-y-8 animate-fade-in relative z-10 max-w-[1920px] mx-auto w-full">
                {/* Header Section */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div className="animate-slide-right delay-100">
                        <Breadcrumbs items={[{ label: 'Dashboard', path: '/' }]} />
                        <h1 className="text-4xl md:text-5xl font-heading font-black text-text-main mt-4 tracking-tight">
                            {t('welcome')}, <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-indigo-600 animate-gradient-x">{user?.name}</span>
                        </h1>
                        <p className="text-lg text-text-muted mt-2 font-medium opacity-80">{t('overview')}</p>
                    </div>

                    <div className="flex gap-4 animate-slide-left delay-200">
                        <button
                            onClick={() => setIsSettingsOpen(true)}
                            className="flex items-center gap-2 px-5 py-3 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-sm border border-slate-200 dark:border-slate-700 font-bold"
                        >
                            {ICONS.settings} Clinic Settings
                        </button>
