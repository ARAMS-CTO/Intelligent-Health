        ...
        <div className="container mx-auto px-4 py-8 space-y-8 animate-fade-in">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    ...
                    >
                        {ICONS.ai} Analyze System
                    </button>
                    <div className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-colors ${stats.systemHealth === 'Optimal' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className={`glass-card p-6 rounded-2xl border-l-4 ${stats.systemHealth === 'Optimal' ? 'border-green-500' : 'border-yellow-500'}`}>
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-text-muted text-xs font-bold uppercase tracking-wider">Service Health</p>
                            <p className="text-3xl font-heading font-bold text-text-main mt-1">{stats.systemHealth}</p>
                        </div>
                        <div className={`p-2 bg-slate-100 dark:bg-slate-800 rounded-lg ${stats.systemHealth === 'Optimal' ? 'text-green-500' : 'text-yellow-500'}`}>
                            {stats.systemHealth === 'Optimal' ? ICONS.check : ICONS.riskModerate}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <Modal isOpen={isAiModalOpen} onClose={() => setIsAiModalOpen(false)}>
                <div className="w-[800px] bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-2xl">
                    <SpecialistAgentChat zone="admin" contextId="system_health" />
                </div>
            </Modal>
        </div>
    );
