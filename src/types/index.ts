...
export interface DoctorProfile {
  ...
  website?: string;
  openingHours?: string; // or structured object
...
export interface ToothStatus {
  id: string;
  tooth_number: number;
  condition: 'healthy' | 'decay' | 'filled' | 'crown' | 'missing' | 'implant' | 'root_canal';
  surface?: string;
  notes?: string;
  last_treated?: string;
}
...
export type DentalCategory = 'Esthetic' | 'Therapeutic' | 'Preventive' | 'Orthodontic';
...
export interface ImagingStudy {
  ...
  status: 'Pending' | 'In Progress' | 'Reported' | 'Reviewed';
  orderedAt: string;
